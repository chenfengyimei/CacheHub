...
public class PayloadAndExportTests
{
    ...
    public async Task FileExporter_ExportToRepositoryAsync_ShouldCreateCachehubDir()
    {
        ...
        Assert.True(File.Exists(Path.Combine(cachehubDir, "latest-context.md")));

        // .gitignore should contain .cachehub/
        var gitignore = File.ReadAllText(Path.Combine(repoDir.Path, ".gitignore"));
        Assert.Contains(".cachehub/", gitignore);
    }
    ...
    public async Task FileExporter_ExportToRepositoryAsync_ShouldNotDuplicateGitignoreEntry()
    {
        using var tempDir = new TempDir();
        using var repoDir = new TempDir();
        await File.WriteAllTextAsync(Path.Combine(repoDir.Path, ".gitignore"), ".cachehub/\n");
    ...
    private sealed class TempDir : IDisposable
    {
        ...
        public TempDir()
        {
            Path = System.IO.Path.Combine(System.IO.Path.GetTempPath(), "cachehub_export_" + Guid.NewGuid().ToString("N"));
