...
public class R7GateRegressionTests
{
    ...
    // R7 Gate: Process restart → safe cache still hits
    [Fact]
    public void Gate_RestartSafe_CacheStillHits()
    {
        var factory = SetupFactory();

        // Simulate "first process" — put into cache
        ...
        // Simulate "second process" — new store instance, same DB
    ...
    // R7 Gate: File/policy change → dependency cache correctly invalidates
    [Fact]
    public void Gate_DependencyChange_InvalidatesCache()
    {
        ...
        // Same dependency hash → hit
        Assert.NotNull(store.TryGet("dep-test", CacheType.Context, "file-hash-v1"));

        // Different dependency hash (file changed) → miss + invalidated
    ...
    // R7 Gate: Failed/partial streaming/tool calls → cannot enter cache
    [Fact]
    ...
    // R7 Gate: Cache corruption → auto-isolate and fall back
    [Fact]
    ...
    // R7 Gate: TTL expiry → automatic invalidation
    [Fact]
