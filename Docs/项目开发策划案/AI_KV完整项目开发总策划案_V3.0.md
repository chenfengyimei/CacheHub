...
本文件将此前的 AI_KV 产品策划、模块化 V2 修订、评审建议，以及“任何 Agent 主动接入 AI_KV”的通用接入修订整合为一份统一总纲。它既描述最终产品全貌，也明确核心验证版、技术用户版、完整桌面版和长期生态版之间的开发边界。
...
- AI_KV 不局限于 Codex，也不局限于传统 Coding Agent；任何能调用 API、执行本地命令、读取文件或读取教程的 AI 应用都可接入。
- 官方不为无限数量的 Agent 编写强绑定适配器；官方维护稳定协议、通用 Skill、自动接入教程、示例配置和验证工具。
- Agent 应主动读取 AI_KV 接入教程，并选择 CLI、本地 HTTP API、文件导出或模型 Gateway 完成接入。
...
| --- | --- |
| 项目名称 | AI_KV |
| 推荐副标题 | AI_KV Context Hub / AI_KV Local Context Engine |
...
AI_KV 中的 KV 表示文件、解析、搜索、上下文、工具结果和模型请求等 Key-Value 缓存能力。它不宣称能够生成、保存或向云端提供商注入模型内部的 Attention KV Cache。
...
> 缓存文件并不自动等于节省模型 Token。只有当 AI_KV 在 Agent 构造最终 Prompt 之前提供更小、更相关的 Context Package，并且 Agent 不再重复发送完整项目内容时，才会真正降低输入 Token。

## 2.3 核心产品假设

AI_KV 能否在不明显降低任务成功率的情况下，通过版本感知、可解释和 Token 受限的上下文选择，降低整个任务生命周期中的模型输入 Token 与重复代码读取。
...
- Agent 读取通用 Skill 后，自动调用 AI_KV 检查能力、导入仓库并建立索引。
...
| 模块 | 职责 | 不得承担 |
| --- | --- | --- |
| AiKv.Core | 值对象、错误、配置、模块生命周期 | 不包含 UI、Agent 或 Provider 私有逻辑 |
| AiKv.Storage | SQLite、迁移、原子快照 | 不决定检索策略 |
| AiKv.Workspaces | 工作区、仓库、组件和状态 | 不执行项目脚本 |
| AiKv.Indexing | 扫描、哈希、FTS、增量索引 | 不调用模型生成摘要 |
| AiKv.Parsing | 文本与语法结构 | 不把启发式关系冒充精确语义 |
| AiKv.Context | 召回、排序、预算、Context Package | 不直接执行代码修改 |
| AiKv.Repository | 受控 Git 操作 | 不自动 merge/rebase/reset/push |
| AiKv.Integration | 协议、Skill、教程、脚本 | 不绑定 Agent 私有协议到核心 |
| AiKv.Gateway | 模型转发、统计和安全缓存 | 不猜工作区、不读取额外源码 |
| AiKv.Security | 路径、外发、审批、审计 | 不能被 Agent 绕过 |
| AiKv.Desktop | 用户交互和可视化 | 不直接读写数据库 |

## 8.1 初期避免过度物理拆分

0.1-alpha 可先使用 AiKv.Core、AiKv.Storage、AiKv.Indexing、AiKv.Context、AiKv.Cli、AiKv.Tests 六个项目。只有当模块出现独立发布、独立权限或独立变化周期时，再拆分 Repository、Integration、Desktop、Gateway 和 Parsing 插件程序集。
...
工作区是 AI_KV 的最高项目容器，可以包含单个仓库、Monorepo、多仓库集合或非 Git 目录。缓存、安全策略、索引快照、Context Package 和统计均以工作区为隔离边界。
...
忽略规则依次合并系统默认、.gitignore、.aikvignore 和工作区 GUI 规则。每次构建 Context Package 必须记录 ignore_rules_hash。
...
> Context Package 是在确定工作区版本、任务、安全策略和 Token 预算下，由 AI_KV 选择并封装的可解释、可复现最小代码上下文。

## 12.2 Manifest 与 Payload 分离

- Manifest：文件、范围、哈希、版本、理由、预算、安全和排除信息。
- Payload：实际代码、Outline、Diff、Repo Map 和格式包装。
- 客户端可以只读取 Manifest 后自行读取文件，也可以要求 AI_KV 返回完整 Payload。
...
```text
aikv context expand <context-id> --symbol UserRepository
aikv context expand <context-id> --file src/auth/repository.ts
aikv context expand <context-id> --reason "缺少刷新令牌持久化实现"
```
...
> AI_KV 提供稳定协议、通用 Skill 和教程；Agent 读取教程并主动接入 AI_KV。官方不追着无限适配每个 Agent。
...
│  └─ tool-instruction.md
├─ scripts/
│  ├─ install-aikv-integration.ps1
│  ├─ install-aikv-integration.sh
│  └─ verify-integration.*
├─ tutorials/
│  ├─ api-only.md
│  ├─ cli-agent.md
...
Codex、Claude Code、Cursor、Cline、Aider 等教程只展示如何利用公共协议，不构成 AI_KV Core 的依赖。Agent 更新最多影响对应教程和示例，不应修改 Context Package Schema 或核心接口。
...
aikv workspace detect --path <path>
aikv workspace import --path <path>
aikv workspace status --workspace <id>
aikv workspace refresh --workspace <id>

aikv context build --workspace <id> --task "..." --budget 16000
aikv context inspect <context-id>
aikv context export <context-id> --format markdown
aikv context expand <context-id> --symbol <name>
aikv context feedback <context-id> --file feedback.json

aikv repo inspect <url>
aikv repo clone <url> --destination <path>
aikv repo pull --workspace <id> --strategy ff-only
...
默认导出到 AI_KV 数据目录。只有用户明确开启时，才在仓库内生成 .aikv，并应提供 .gitignore 建议。
...
{
  "metadata": {
    "aikv_workspace_id": "ws_xxx",
    "aikv_context_package_id": "ctx_xxx",
    "aikv_dirty_state_fingerprint": "...",
    "aikv_context_engine_version": "0.1.0"
  }
}
```
...
仅知道生成了哪些文件不足以改进排序。AI_KV 需要知道 Agent 实际使用、额外读取、修改和测试了什么，以判断遗漏、噪声和总工作流 Token。
...
| Compression Ratio | AI_KV 总工作流输入 Token / 基线总输入 Token |
...
- 每次任务从干净快照开始，基线组和 AI_KV 组不共享构建产物或缓存。
...
| AI_KV Test Pass Rate | 不低于基线的 95% |
...
- aikv context build/inspect/export/expand
- Context API
...
| 能力 | AI_KV | Agent 内置索引 | 模型 Gateway | 通用代码搜索 |
| --- | --- | --- | --- | --- |
| 多客户端共享 | 是 | 通常否 | 部分 | 部分 |
...
> AI_KV 是面向所有 AI 编程客户端和代码自动化应用的本地上下文基础设施。它通过版本感知索引、可解释 Context Package、Token Budget、稳定公共协议和可选模型 Gateway，为任何能够调用 API、执行本地命令或读取教程的工具提供代码上下文服务。
...
| AI_KV 估算 Token | 指定 tokenizer/version 的本地估算 |
| 实际缓存避免 Token | 完全未发送到 Provider 的请求 Token |
| 上下文压缩估算 | 基线读取和 AI_KV 全生命周期 Token 差异 |
