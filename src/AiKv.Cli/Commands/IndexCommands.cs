...
using AiKv.Core.Indexing;
using AiKv.Core.Jobs;
...
public static class IndexCommands
{
    public static async Task<int> HandleAsync(string[] args)
    {
        if (args.Length == 0)
        {
            Console.WriteLine("Usage: aikv index <build|refresh|status|verify> [options]");
            return 1;
        }
    ...
    private static async Task<int> BuildAsync(string[] args)
    {
        ...
        var ignoreEngine = new IgnoreRuleEngine()
            ...
            .WithAiKvIgnore(Path.Combine(workspace.RootPath, ".aikvignore"));
