...
public static class WorkspaceCommands
{
    ...
    private static async Task<int> HandleWorkspaceImportAsync(string[] args)
    {
        if (args.Length == 0)
        {
            ...
            Console.Error.WriteLine("Usage: cachehub workspace import <path> [--name <name>]");
            return 1;
        }
    ...
    private static async Task<int> HandleWorkspaceRemoveAsync(string[] args)
    {
        ...
        Console.WriteLine("Note: Only CacheHub data was removed. Source code is untouched.");
        return 0;
    }
    ...
    {
        Console.WriteLine("CacheHub - Local code context infrastructure");
        Console.WriteLine();
        Console.WriteLine("Usage: cachehub <command> [options]");
        Console.WriteLine();
